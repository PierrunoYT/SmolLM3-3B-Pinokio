  ...
  run: [
    // windows nvidia
    {
      ...
      params: {
        ...
        message: "uv pip install torch==2.7.0 torchvision==0.22.0 torchaudio==2.7.0 --index-url https://download.pytorch.org/whl/cu128 {{args && args.force_reinstall ? '--force-reinstall --no-deps' : ''}}"
      },
      next: null
    },
    {
      when: "{{platform === 'win32' && gpu === 'nvidia' && args && args.xformers}}",
      method: "shell.run",
      params: {
        venv: "{{args && args.venv ? args.venv : null}}",
        path: "{{args && args.path ? args.path : '.'}}",
        message: "uv pip install xformers"
      },
      next: null
    },
    {
      ...
      params: {
        "venv": "{{args && args.venv ? args.venv : null}}",
        "path": "{{args && args.path ? args.path : '.'}}",
        message: "uv pip install -U triton-windows"
      },
      next: null
    },
    {
      ...
      params: {
        venv: "{{args && args.venv ? args.venv : null}}",
        path: "{{args && args.path ? args.path : '.'}}",
        message: "uv pip install https://github.com/woct0rdho/SageAttention/releases/download/v2.1.1-windows/sageattention-2.1.1+cu128torch2.7.0-cp310-cp310-win_amd64.whl"
      },
      next: null
    },
    // windows amd
    ...
    {
      when: "{{platform === 'linux' && gpu === 'nvidia' && args && args.xformers}}",
      method: "shell.run",
      params: {
        venv: "{{args && args.venv ? args.venv : null}}",
        path: "{{args && args.path ? args.path : '.'}}",
